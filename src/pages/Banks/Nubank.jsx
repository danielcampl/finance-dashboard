...
export default function Nubank() {
  const data = localStorage.getItem('transactionsNubank');
  ...
  const handleAdd = (transaction) => {
    const newArrayTransactionsNubank = [...transactionsList, transaction];
    setTransactionsList(newArrayTransactionsNubank);
    localStorage.setItem('transactionsNubank', JSON.stringify(newArrayTransactionsNubank));
  }
    ...
    <section className='painel-container'>
      <div className='header-content'>
        <IoArrowBackCircle
          ...
          onClick={dashboard}
        />
        <h1>Controle Financeiro Nubank</h1>
        <IoLogOut
          className='logout-icon'
          onClick={backToLogin}
        />
