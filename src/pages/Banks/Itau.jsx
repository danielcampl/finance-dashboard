...
export default function Itau() {
  const data = localStorage.getItem('transactionsItau');
  ...
  const handleAdd = (transaction) => {
    const newArrayTransactionsItau = [...transactionsList, transaction];
    setTransactionsList(newArrayTransactionsItau);
    localStorage.setItem('transactionsItau', JSON.stringify(newArrayTransactionsItau));
  }
    ...
    <section className='painel-container'>
      <div className='header-content'>
        <IoArrowBackCircle
          ...
          onClick={dashboard}
        />
        <h1>Controle Financeiro Itau</h1>
        <IoLogOut
          className='logout-icon'
          onClick={backToLogin}
        />
