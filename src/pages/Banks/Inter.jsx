...
export default function Inter() {
  const data = localStorage.getItem('transactionsInter');
  ...
  const handleAdd = (transaction) => {
    const newArrayTransactionsInter = [...transactionsList, transaction];
    setTransactionsList(newArrayTransactionsInter);
    localStorage.setItem('transactionsInter', JSON.stringify(newArrayTransactionsInter));
  }
    ...
    <section className='painel-container'>
      <div className='header-content'>
        <IoArrowBackCircle
          ...
          onClick={dashboard}
        />
        <h1>Controle Financeiro Inter</h1>
        <IoLogOut
          className='logout-icon'
          onClick={backToLogin}
        />
